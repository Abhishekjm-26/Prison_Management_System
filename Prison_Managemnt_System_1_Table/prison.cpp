...
class prison
{
	...
	string prison_id,prison_name,iid,iname,age,nationalism,crime,hstatus,datein,buffer;
...
{
	buffer.erase();
	buffer+=prison_id+"|"+prison_name+"|"+iid+"|"+iname+"|"+age+"|"+nationalism+"|"+crime+"|"+hstatus+"|"+datein+"$\n";
}
	...
	if(ifile)
	{
		...
		else
		{
			...
			fstream fp;
    			...
    			buffer.erase();
    			fp.close();
    			cout<<"\n\nEnter the rrn no to search\n";
			cin>>rrn;
			P.searchprison(rrn);
		}
	}
	else{
		cout<<"\nError! No records inserted yet\n";
	}
}
